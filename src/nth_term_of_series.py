...
def difference_finder(series_list: list):
    """Takes given series as input and returns 
    the difference and checks if the difference is constant"""
    series_diff_list = []
...
def nth_term_of_series(terms_of_series: list):
    ...
    if series_aggregates[1]:
        ...
        for i in range(len(aggregates_first_terms)):
            coeff *= (n - i)
            sum += coeff*aggregates_first_terms[i]/(math.factorial(i) * n)

        return f"General equation of nth term of series is: {sp.simplify(sum)}"

    else:
        print("Terms are not sufficient to find constant difference\nor it is not possible to find the nth term.")

if __name__ == "__main__":
    nth_term_of_series(series)